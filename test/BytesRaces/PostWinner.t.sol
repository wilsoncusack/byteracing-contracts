...
contract PostWinnerTesrt is ByteRacesBaseTest {
    ...
    function test_paysOutCorrectly(uint64 raceRegistrationFee_, uint8 creatorTakePercent_) public {
      ...
      _setupWinnerPostableRace();
      uint totalFees = byteRaces.raceDetails(raceId).totalFees;
      uint creatorTake = totalFees * creatorTakePercent / byteRaces.CREATOR_TAKE_DENOMINATOR();
      uint winnerTake = totalFees - creatorTake;
      uint creatorBeforeBalance = byteRaces.raceDetails(raceId).creator.balance;
      uint winnerBeforeBalance = payoutAddress.balance;
      byteRaces.postWinner(raceId, winner);
    ...
    function _setupWinnerPostableRace() internal {
      ...
      vm.prank(racerOwner);
      byteRaces.registerRacerForRace{value: raceRegistrationFee}({raceId: raceId, racerId: winner, payoutTo: payoutAddress});
